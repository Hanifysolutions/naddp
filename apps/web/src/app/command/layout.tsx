...
/**
 * The command-centre chrome lives in a layout rather than in the page so that it persists
 * across navigation and, critically, so `loading.tsx` renders *inside* it. A route
 * transition must never blank the DEMO badge or the identity control, even for a frame.
 *
 * Identity is resolved here, once, on the server: `GET /v1/session/me` with the browser's
 * cookie. Two things are built from that single answer - the navigation rail, and the
 * client-side `SessionProvider` the tiles read. One lookup means the rail and the board can
 * never disagree about what this role holds.
 */
