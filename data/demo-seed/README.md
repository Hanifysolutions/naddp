...
| `manifest.json` | seed track | Generated by `make seed`, git-ignored. Maps stable seed slugs to the primary keys of the current dataset, so eval cases and snapshots can reference `sig-covalent-kwinana-refinery-rampup` rather than a UUID. Keys are a pure function of the slug and so are identical on every run and every machine; the `cases.public_ref` values it also records are CSPRNG-minted (ADR-0007) and change on each `make demo-reset`. |
