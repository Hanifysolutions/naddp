...
> **This README was written ahead of the code it describes, and the code has now landed.**
> `seed.py` (with its `seed_parts/` modules), `citations.json` and `ai_snapshots/` are all present.
> This file remains the contract they are held to; where the two disagree, the Build Bible and
> `docs/OPEN_QUESTIONS.md` decide.
...
| `seed.py` | seed track | Entry point for `make seed`. Loads the full synthetic dataset. Idempotent (upsert-only); safe to re-run. |
| `seed_parts/` | seed track | The dataset itself, one module per bounded context, plus the determinism/identity/idempotency plumbing in `context.py`. |
...
| `manifest.json` | seed track | Generated by `make seed`, git-ignored. Maps stable seed slugs to the primary keys of the current dataset, so eval cases and snapshots can reference `sig-covalent-kwinana-refinery-rampup` rather than a UUID. Keys are a pure function of the slug and so are identical on every run and every machine; the `cases.public_ref` values it also records are CSPRNG-minted (ADR-0007) and change on each `make demo-reset`. **The running API never reads this file.** It is a convenience for humans and offline tooling: anything in `apps/api` that needs a seeded row's id calls `app.core.ids.seed_id(slug)` and computes it, because a git-ignored file is absent from the deployed image and cannot be depended on at request time. |
...
|---|---|---|
| Signal | intelligence | Australia scaling lithium **midstream/downstream** against a documented processing-skills gap, citing real public sources. Anchor: Covalent Mt Holland-Kwinana — the *refinery* is RAMPING toward its ~50 ktpa nameplate while the *concentrator* has an approved expansion. **Never write that an Australian lithium refinery is expanding** — none is (`BUILD_BIBLE.md` §2, `OPEN_QUESTIONS` Q-16). This row previously said "refinery expansion"; corrected by the seed track when Q-16 was resolved. |
...
- The chain must survive `make demo-reset` **and a redeploy**. Reference a link by its slug, never by
  a copied-out UUID: the Unified Outcomes board computes its anchors from the committed slugs in
  `hero_thread.json` with `app.core.ids.seed_id`, so the corridor resolves with nothing on disk but
